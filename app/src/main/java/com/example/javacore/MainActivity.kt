...
class MainActivity : AppCompatActivity(), IonClick {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        students.add(
            Student(
                "Hà Văn Hoàng", 1998, "0399408879",
                "CNTT", "Đại học"
            )
        )
        students.add(
            Student(
                "Bùi Văn Quang", 1999, "0123456789",
                "CNTT", "Cao Đẳng"
            )
        )
        students.add(
            Student(
                "Phạm Minh Hiếu", 1999, "0321654897",
                "CNTT", "Đại học"
            )
        )
        students.add(
            Student(
                "Bùi Xuân Trường", 2000, "0987654321",
                "CNTT", "Cao Đẳng"
            )
        )
        listToSort = students
        ...
        btn_Add.setOnClickListener {
            ...
            if (ed_Name.text.toString().isEmpty() || ed_Major.text.toString().isEmpty()
                ...
                Toast.makeText(
                    this@MainActivity,
                    "Không được để trống bất kì trường nào",
                    Toast.LENGTH_LONG
                ).show()
            } else if (ed_YearOfBirth.text.toString().length != 4) {
                Toast.makeText(this@MainActivity, "Năm sinh phải có 4 chữ số", Toast.LENGTH_LONG)
                    .show()
            } else if (ed_PhoneNumber.text.toString().length != 10
                || !ed_PhoneNumber.text.toString().substring(0, 1).equals("0")
            ) {
                Toast.makeText(
                    this@MainActivity, "Số điện thoại có 10 chữ số và bắt đầu bằng chữ số 0",
                    Toast.LENGTH_LONG
                ).show()
            } else {
                var student: Student = Student(
                    ed_Name.text.toString(), ed_YearOfBirth.text.toString().toInt(),
        ...
        btn_Update.setOnClickListener {
            ...
            if (!phoneNumber.isEmpty()) {
                ...
                if (ed_Name.text.toString().isEmpty()
                    || ed_Major.text.toString().isEmpty()
                    || ed_PhoneNumber.text.toString().isEmpty()
                    || ed_YearOfBirth.text.isEmpty()
                ) {
                    Toast.makeText(
                        this@MainActivity, "Không được để trống bất kì trường nào",
                        Toast.LENGTH_LONG
                    ).show()
                } else if (ed_YearOfBirth.text.toString().length != 4) {
                    Toast.makeText(
                    ...
                    Toast.makeText(
                        this@MainActivity,
                        "Số điện thoại có 10 chữ số và bắt đầu bằng chữ số 0",
                        Toast.LENGTH_LONG
                    ).show()
                } else {
                    var student: Student = Student(
    ...
    fun SortByName() {
        var sortByName: List<Student> = listToSort.sortedBy {
    ...
    fun Search(strSearch: String?): MutableList<Student> {
        ...
        for (i in 0 until students.size) {
            if (students[i].education.toLowerCase().contains(str)
                || students[i].yearOfBirth.toString().contains(str)
                || students[i].name.toLowerCase().contains(str)
                || students[i].major.toLowerCase().contains(str)
